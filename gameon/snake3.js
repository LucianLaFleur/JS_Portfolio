...
// key click button for showing the key didn't fucking work
// var ptsKeyBtn = document.getElementById('ptsKeyBtn');
...
// console.log(`new food arr item 2 = ${foodCoordsArr[1].value}`)

// give a placeholder for the newly spawned item's value
let newSpawnVal;
  ...
  for (var i = 0; i < 3; i++){
    while (true) {
        ...
        // Make sure no two foods spawn on the same coords
            if(!fucked){
                // ctx.drawImage(spawnedFood, foodCoords.x, foodCoords.y, 32, 32);
...
function makeSingleFood(i){
  ...
  while (true) {
        ...
        // assign generated coords to their place in the higher-scoped arr
        foodCoordsArr.splice(i, 1, foodObj);
        // foodCoordsArr[foodCoordsArr.indexOf(foodObj)] = foodObj;

            // console.log("X val from single-generate function:" + foodObj.x + " and Y:" + foodObj.y);
            // console.dir(foodCoordsArr);
        snake.forEach(function (dummy){
            if((dummy.x == foodCoordsArr[i].x) && (dummy.y == foodCoordsArr[i].y)){
...
// at end of coincidence section, generate food values
//   generateFoodVal(i);
  } 
  }
  if(!foodTouched){
